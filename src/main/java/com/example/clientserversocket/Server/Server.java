...
public class Server {
    ...
    private static final String DB_URL = "jdbc:postgresql://localhost:5432/postgre";
    ...
    private static class ClientHandler implements Runnable {
        ...
        private void deleteFile(DataInputStream dis, DataOutputStream dos) throws IOException {
            String fileName = dis.readUTF(); // Lê o nome do arquivo a ser deletado do stream de entrada.
            try (Connection conn = DriverManager.getConnection(DB_URL, DB_USER, DB_PASSWORD)) {
                String filePath = null;
                // Prepara e executa uma consulta SQL para encontrar o caminho do arquivo com base em seu nome.
                String sql = "SELECT path FROM files WHERE name = ?";
                try (PreparedStatement pstmt = conn.prepareStatement(sql)) {
                    pstmt.setString(1, fileName);
                    ResultSet rs = pstmt.executeQuery();
                    if (rs.next()) {
                        filePath = rs.getString("path"); // Armazena o caminho do arquivo, se encontrado.
                    }
                }

                if (filePath != null) {
                    File file = new File(filePath);
                    if (file.delete()) { // Tenta deletar o arquivo do sistema de arquivos.
                        // Se o arquivo foi deletado com sucesso, remove a entrada correspondente no banco de dados.
                        ...
                        try (PreparedStatement deleteStmt = conn.prepareStatement(sql)) {
                            ...
                            if (affectedRows > 0) {
                                ...
                                dos.writeUTF("Erro: Arquivo não encontrado no banco de dados.");
                            }
                        }
                    } else {
                        dos.writeUTF("Erro: Falha ao deletar arquivo do armazenamento do servidor.");
                    }
                } else {
                    dos.writeUTF("Erro: Arquivo não encontrado no banco de dados.");
                }
