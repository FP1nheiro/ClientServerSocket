...
public class Client extends Application {
    ...
    public void start(Stage primaryStage) throws Exception {
        ...
        Button deleteButton = new Button("Deletar Arquivo Selecionado");
        ...
        deleteButton.setOnAction(event -> {
            String selectedFile = fileList.getSelectionModel().getSelectedItem();
            if (selectedFile != null) {
                deleteFile(selectedFile); // Chama o método para deletar o arquivo selecionado.
            }
        });

        // Configuração inicial da interface do usuário.
        fileList.setItems(observableList);
        VBox layout = new VBox(10, fileList, uploadButton, downloadButton, deleteButton);
    ...
    private void deleteFile(String fileName) {
        new Thread(() -> {
            try (Socket socket = new Socket(SERVER_ADDRESS, SERVER_PORT);
                 DataOutputStream out = new DataOutputStream(socket.getOutputStream());
                 DataInputStream in = new DataInputStream(socket.getInputStream())) {
                out.writeUTF("DELETE"); // Envia o comando de DELETE para o servidor.
                out.writeUTF(fileName); // Envia o nome do arquivo a ser deletado.

                String response = in.readUTF(); // Aguarda a resposta do servidor.
                Platform.runLater(() -> exibirAlerta("Resposta do Servidor", response)); // Exibe a resposta do servidor em um alerta.
                atualizarListaArquivos(); // Atualiza a lista de arquivos após a deleção.
            } catch (IOException e) {
                Platform.runLater(() -> exibirAlerta("Erro", "Falha ao deletar o arquivo: " + e.getMessage()));
                e.printStackTrace();
            }
        }).start();
    }
